...
import com.rrs.corona.test.ObjectFactory;
...
//	private transient FileInputStream fin = null;
	...
	public XMLCreater(final String filePath)
	{
		try
		{
			jaxbContext = JAXBContext.newInstance("com.rrs.corona.test");
	...
	public void createXML()
	{
		try
		{
			...
			for(int i=0;i<6;i++)
			{
				...
				Info empType = objectFactory.createInfo();
				empType.setAdrs("Bangalore"+i);
				empType.setDesgn("Engineer"+i);
				empType.setName("Deba"+i);
				writeToXML(empInfo , jaxbContext , fout);
				//empInfo.getInfo().setAdrs("sdsjfdsj");
				//(empInfo.getInfo()).add(empType);
			}
//			Info empType = objectFactory.createInfo();
//			empType.setAdrs("Bangalore");
//			empType.setDesgn("Engineer");
//			empType.setName("Deba");
//			writeToXML(empInfo , jaxbContext , fout);
		}
		catch(Exception e)
		{
...
//	}
	...
	{
		XMLCreater creater = new XMLCreater("F:/TestData/XMLJAXB_V1/test_dtd.xml");
		creater.createXML();
